...
ApprovedRecord = ApprovedChange | ApprovedWorkspaceChange
...
def change_hash(change: ApprovedRecord) -> str:
    return stable_hash(
        change.model_dump(
            mode="json",
            by_alias=True,
...
def change_references(
    changes: list[ApprovedRecord],
) -> tuple[ApprovedChangeReference, ...]:
    return tuple(
        ApprovedChangeReference(
...
def ledger_hash(changes: list[ApprovedRecord]) -> str:
...
def issue_approval(changes: list[ApprovedRecord]) -> ApprovalRecord:
